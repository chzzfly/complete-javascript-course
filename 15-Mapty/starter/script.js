...
// 建4个类，prettier一直报错？因为把super写在constructor外面了
...
class App {
  ...
  constructor() {
    ...
    // 从local storage 获取用户的数据
    this._getLocalStorage();

    // 用户提交事件。事件监听函数调用的回调函数“this._newWorkout"，它里面的this会指向form元素，而不是 App class 创建的对象，我们需要重新指向
  ...
  _setLocalStorage() {
    // key,value，不应该用localstorage存储大量数据
